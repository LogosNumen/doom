...
filtered noise, slow LFOs, and `speechSynthesis`. The music is generated by
`js/audio/`; design notes and a per-track breakdown are in
**[MUSIC_NOTES.md](MUSIC_NOTES.md)**.

### The shape of the engine

```
js/audio/
  theory.js    seeded PRNG, modes, chords, voice leading, Euclidean rhythm
  fx.js        generated impulse responses, reverb, ping-pong delay, chorus,
               saturation, bit crush, tape wow, hiss, master chain
  voices.js    pad, organ, pluck (Karplus-Strong), FM bell, vowel, sub,
               noise perc, kick
  engine.js    context, buses, transport, voice allocation, crossfade,
               metering, session continuity, speech ducking
  tracks/      one file per track: an arrangement table and a generator
```

Nothing under `js/audio/` is loaded until somebody turns sound on — `sound.js`
injects the thirteen files in dependency order on first use, so a reader who
never touches the ♫ never downloads any of it.

Scheduling is the lookahead pattern: a 25 ms interval schedules everything
falling in the next 100 ms against `audioContext.currentTime`. Notes are never
fired from a timer directly, because the drift is audible within a couple of bars.

### Adding a track

Copy any file in `js/audio/tracks/` — `relay.js` is the most representative —
and change three things.

**1. The arrangement table.** This is the piece's shape, as data, at the top of
the file:

```js
const ARRANGEMENT = [
  { bar: 0,  name: "alone",      layers: ["arp"] },
  { bar: 16, name: "everything", layers: ["arp", "bass", "pad", "lead"] },
  { bar: 48, name: "out",        layers: ["pad"] }
];
const LOOP_AT = 96, LOOP_TO = 16;   // past bar 96, fold back to bar 16
```

**2. The definition.** `id`, `title`, `bpm` (55–90), `root` as a MIDI number,
`mode` (`aeolian`, `dorian`, `phrygian`, `lydian`), the hiss level, and the
delay time — `60 / bpm * 0.75` is the dotted eighth and is usually the one you
want.

**3. `init` and `onStep`.** `init` returns whatever state the track needs
(progressions, Euclidean patterns, a wow LFO). `onStep` is called for every
sixteenth-note step with the exact time it should sound at:

```js
onStep: function (ctx, bus, st, step, t, eng) {
  const bar = Math.floor(step / 16), pos = step % 16;
  const sec = U.sectionAt(ARRANGEMENT, U.loopBar(bar, LOOP_AT, LOOP_TO));
  if (U.has(sec, "bass") && pos % 4 === 0) {
    eng.note("sub", { t: t + U.jitter(st.rng, 8), f: T.mtof(48),
                      gain: U.vel(st.rng, 0.4), bus: bus });
  }
}
```

Then add the filename to `FILES` in `js/sound.js` and to the script list in
`tools/render_harness.html`.

Two things worth knowing before you tune levels. **Set gains deliberately** —
a foreground, a middle and a background; everything at 0.5 is why generative
audio turns to mush. And **keep something in the mid in every section**: a
stretch of kick and sub alone measures as 90%+ below 200 Hz and sounds exactly
as bad as that reads.

### Rendering and checking it

Since nobody writing this can hear it, the tracks are rendered offline and
measured:

```bash
python tools/render_previews.py          # WAVs into previews/ (gitignored)
python tools/analyse_audio.py            # the table
```

`render_previews.py` drives `tools/render_harness.html` in headless Chromium,
rendering each track once through an `OfflineAudioContext` and cutting three
windows out of it — 0–30 s, 60–90 s and 240–270 s. The last one is the point: if
240 s measures the same as 0 s, the track is a loop wearing a costume.

`analyse_audio.py` checks clipping, peak and RMS, unintended silence, DC offset,
clicks, stereo correlation, spectral balance and level drift, then compares the
0 s and 240 s windows per track. Needs `playwright` and `numpy`
(`pip install -r tools/requirements.txt`, then `python -m playwright install chromium`).
